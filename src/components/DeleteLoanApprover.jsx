...
export default function DeleteLoanApprover(props) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (props.name === "DeleteHealthApplication") {
            try {
                const response = await axios.delete("http://localhost:9090/deleteLoanapprover", {
                    data: email,
                    headers: {
                        "Content-Type": "text/plain",
                    },
                });
                console.log("Successfully deleted Loanapprover:", response.data);
                alert("Successfully deleted Loanapprover:", response.data);
            } catch (error) {
            ...
            try {
                ...
                const response = await axios.get("http://localhost:9090/getLoanapprover", {
                    params: { email },
                    headers: {
